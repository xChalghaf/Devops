node {
    ...
    stage('sonar-scanner') {
      ...
      withCredentials([string(credentialsId: 'sonar', variable: 'sonarLogin')]) {
        sh "${sonarqubeScannerHome}/bin/sonar-scanner -e -Dsonar.host.url=http://127.17.0.3:9000 -Dsonar.login=05ed0a5f4adda16f4b394b0296f74e9eadb27b7b -Dsonar.projectName=ayoubmahou -Dsonar.projectVersion=${env.BUILD_NUMBER} -Dsonar.projectKey=GS -Dsonar.sources=src/main/ -Dsonar.tests=src/test/ -Dsonar.java.binaries=build/**/* -Dsonar.language=java"
      }
    }
